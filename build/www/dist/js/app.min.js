    ...
    function() {
        ...
        function() {
            ...
            function BonziTextNameDirective($log, $timeout, name) {
                return {
                    scope: {
                    ...
                    link: function(scope, element) {
                        ...
                        scope.message.type && "text" !== scope.message.type || (scope.bonzi = scope.message._bonzi || _.get(window.bonzis, ""), scope.selector = `#cl-msg-${scope.message.id} > .bonzi-name`, scope.bonzi && $timeout(hookupContextMenu, 100)), scope.$on("$destroy", function() {
            ...
            function ChatLogController($rootScope, $scope, $log, $compile, $http, $timeout, $interval, $window, socket) {
                ...
                $rootScope.muteAudio = vm.status.muted, $rootScope.backgroundMuted = vm.status.backgroundMuted, $scope.options = {
                    ...
                    vm.initialized = !0, $scope.addEventMessage({
                        ...
                        source: "BonziWORLD",
                        event: "discord",
                        text: `<span class="log-discord">Bwnzi-world.tk now has a Discord! Join at: <a class="discord-link log-discord-link" href="${window.discord.defaultInvite}" target="_blank">${window.discord.defaultInvite}</a>.</span>`,
                        noFilter: !0
                    })
                }, vm.displayTime = function(date) {
                  //Hack
                    if (!date) return moment(date).format("HH:mm:ss") + " | " + " User "
