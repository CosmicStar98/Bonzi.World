            ...
            _createClass(Bonzi, [
                ...
                },
                {
                    ...
                    value: function (data) {
                        var self = this;
                        if (data && data.src) {
                            if (data && data.src && !data.src.mp4 && data.src.gif) return console.dir("do it gaylord"), void self.image({ url: data.src.gif });
                            if (!this.mute) {
                                var thisDialogId = s4();
                                (self.openDialogId = String(thisDialogId)), self.$dialog.addClass("video"), self.$dialog.addClass("autosize");
                                var videlem = document.createElement("video"),
                                    id = `${thisDialogId}_v`;
                                (videlem.id = id), console.dir(data);
                                var mw = Math.floor(window.innerWidth - self.x);
                                mw > 400 && (mw = 400), (mw -= 20), console.log(`mw: ${mw}.`);
                                const playerOptions = { preload: "metadata", height: "auto", width: mw, loop: !0, muted: window.muteAudio || !1 };
                                if (self.player && "function" == typeof self.player.dispose)
                                    try {
                                        self.player.dispose();
                                    } catch (err) {}
                                self.$dialogCont.append(videlem),
                                    setTimeout(function () {
                                        (self.player = videojs(videlem, playerOptions)),
                                            self.player.src({ type: "video/mp4", src: data.src.mp4 }),
                                            self.player.on("dispose", function () {}),
                                            self.player.on("seeked", function () {
                                                0;
                                            }),
                                            self.player.on("loadedmetadata", function () {
                                                var duration = self.player.duration() || 1;
                                                duration <= 3 && self.player.loop(!0), duration <= 1 ? 5 : duration <= 2 ? 3 : duration <= 3 && 2;
                                            }),
                                            self.player.ready(function () {
                                                $(self.$dialog).is(":hidden")
                                                    ? self.$dialog.show(200, function () {
                                                          self.player.play();
                                                      })
                                                    : self.player.play();
                                            }),
                                            ...
                                            setTimeout(function () {
                                                self.player && self.player.dispose;
                                            }, 15e3);
                                    }, 100);
                            }
                        }
